...
| 🔵 **Kubernetes / CKA — Phase 18. VMs 201–205, VIP `.206`, `education/k8s-cka-prep/`** | `phases/phase18_k8s_cka_build.md` **+ `education/k8s-cka-prep/lab-parity.md`** | ✅ **THE CLUSTER EXISTS — Sep 17, 2026. Stage 1 is 5 of 6 steps done:** five nodes `Ready` at `.201`–`.205`, **3 stacked etcd members with a leader**, **kube-vip VIP `.206` up and held by exactly one node**, **Calico v3.32.2 via the Tigera operator on `10.244.0.0/16`**. 🚨 **Four corrections live in the plan's Stage 1 results block and each would mislead you: kube-vip did NOT crash-loop (the fix was applied before init, so the symptom never appeared); the Calico OPERATOR hard-codes `cidr: 192.168.0.0/16` and the docs' "auto-detects with kubeadm" note is on a DIFFERENT TAB; `node.spec.podCIDR` is INERT because Calico IPAM allocates the real addresses; and editing a static pod manifest RECREATES the pod so `RESTARTS 0` does not mean it was untouched.** ✅ **STAGE 2 ALSO COMPLETE: `c02-virgin-cluster` is an OFFLINE baseline on all five and its ROLLBACK IS PROVEN** — markers were planted in etcd and on disk first, because a restore and a plain reboot print the same thing. **Restore costs ~2 min; VIP failover measured at 17 s.** 🔲 **Next: chapters 03 and 04 — Stage 3 does NOT open until they exist.** ✅ **PLAN APPROVED Sep 16, 2026 and STAGE 0 IS COMPLETE** — five nodes live at `.201`–`.205`, identical and **role-less**, kernel **6.8.0-139**, snapshot **`c01-nodes-ready` — verified present at BOTH the PVE and ZFS layers Sep 17; 🔻 the record claimed it from Sep 16 and it did NOT exist, so check the zvol, not the note.** 🔻 **The kubelet is `activating (auto-restart)`, NOT `inactive`** — it dies on a missing `/var/lib/kubelet/config.yaml` until `init`/`join` writes it, and the old `inactive` reading was an artefact of never having rebooted. **Held: `containerd.io cri-tools kubeadm kubectl kubelet`; `crictl` 1.35.0 IS installed (`jq` still deliberately is not).** 🔲 **Stage 1 is next and its first step is NOT `kubeadm init`** — the kube-vip **manifest** goes in place first. ⛔ **There are NO planted traps and none may be added** — withdrawn deliberately at Andrew's instruction, recorded as a `METHOD.md` deviation in §8. 🚨 **`kubeadm init` MUST carry `--control-plane-endpoint` pointing at `.206`:** it fixes the apiserver certificate's SANs at init, so initialising without it means control-2 and control-3 **can never join** without regenerating certs — one of **three** values fixed at `init`. 🚨 **THE OTHER TWO WERE SPECIFIED NOWHERE UNTIL Sep 17, 2026: `--pod-network-cidr=10.244.0.0/16` and `--service-cidr=10.96.0.0/12`.** ⛔ **Calico's DEFAULT pool `192.168.0.0/16` CONTAINS this lab's `192.168.1.0/24`** — left alone it hands pods real LAN addresses and presents as an intermittent CNI or switch fault. ⛔ **Calico must be told the CIDR explicitly: `CALICO_IPV4POOL_CIDR` ships COMMENTED OUT and defaults to `192.168.0.0/16` regardless of what kubeadm was told, so the two can disagree while the cluster looks healthy.** ⭐ **Second appearance of one pattern (see Phase 16's Docker `10.0.0.0/8` finding): a default CIDR is a guess about someone else's network — and at a firm on `10.x` the one to check is Kubernetes' own `10.96.0.0/12`.** 🔻 **CORRECTED Sep 16, 2026 — this row twice said "kube-vip must be ANSWERING before `kubeadm init`". IT CANNOT BE.** kube-vip is a **static pod**, static pods are started by the **kubelet**, and the kubelet **cannot start at all** until `kubeadm init` writes `/var/lib/kubelet/config.yaml` (🔻 sharpened Sep 17 — measured after a reboot it is `activating (auto-restart)`, not `inactive`, and it dies before ever reading the manifests directory, which makes this conclusion stronger). ⭐ **Ordering is manifest-then-init; the VIP comes up DURING init.** ⛔ **Do not wait for `.206` to ping — it never will, and the build will look stuck when nothing is wrong.** ⚠️ Expect kube-vip to crash-loop briefly (its kubeconfig does not exist until init creates it), and note **v1.35 splits `admin.conf` from `super-admin.conf`**. ✅ **VERIFIED Sep 17, 2026 — the correction was RIGHT** (kube-vip's *Static Pods* page documents manifest-then-init). 🚨 **The kubeconfig note beside it was the wrong one: point kube-vip at `admin.conf` on the first control plane and `kubeadm init` TIMES OUT after ~4 min with an unusable cluster.** ⭐ **On control-1 ONLY, set the manifest's `hostPath` to `super-admin.conf` (leave `mountPath` at `admin.conf`) and revert after init; `super-admin.conf` exists only on the FIRST control plane, so cp-2/cp-3 use it as generated.** ⚠️ **Interface is `eth0`, not the docs' `ens160`.** ✅ **kube-vip `v1.2.3`, `--controlplane --arp --leaderElection` only.** 🔻 **Build v1.35, NOT the newest** — 🟢 the exam runs v1.35, about two minors behind upstream, and the upgrade to v1.36 is a deliberate drill. ⛔ **Do NOT patch `setup_docker.sh` to enable containerd's CRI plugin** (hard rule B3) — fix containerd **on the five nodes**; the other eleven lab hosts are Docker hosts that want it disabled, and ⭐ **one build standard cannot serve both a Docker host and a kubeadm node**. ⛔ **`jq` is deliberately NOT installed** — 🟢 the exam ships `yq`; also no `krew`/`k9s`/`kubectx`. ✅ **DHCP pool CONFIRMED `.221–.250`** (Denon landed on `.231` when set back to DHCP; `.230` agrees), so **`.201–.206` will never be offered to a DHCP client.** 🔻 The earlier "pool is unverified" warning was wrong: `.199` had been assigned **by hand** by mistake, not by the router. ⭐ **The real risk in this lab is a human typing a static address into lab range — which is why the reservation is recorded here.** ⚠️ Three household devices do sit at `.215`/`.217`/`.220` on pre-change leases, so **sweep before allocating anything in `.207–.220`.** ⛔ **`.186` (k3s) is FROZEN and out of scope** — never fold the two clusters together. ⚠️ **The moment this cluster exists, this file's statement that "the only Kubernetes in this lab is k3s on VM 186" STOPS BEING TRUE** — rewrite it then, not before, because it is still a correct guard today |
| ⏸️ **Jenkins (VM 185 `.185`), the Jenkins↔GitLab↔Swarm pipeline, `education/jenkins/`** — **ON HOLD since Sep 16, 2026** | `phases/phase17_jenkins.md` | ⏸️ **PAUSED, NOT CLOSED — resume at Part 4.** Everything below still binds while it is on hold, and `.185` keeps running. ⭐ **Weighting is Andrew's and is NOT Phase 16's:** install + configure + wire it up, and above all **Part 6 — bad deployments and recovery — is the centre of gravity.** The Phase 16 security charter is **Part 7 and secondary**; it is also **scoped** — only L21, L22, L12(partial) and the agent's privilege are in scope, because L1/L2/L3 are registry/Swarm work Jenkins cannot fix. 🎭 The phase is written as a **firm-supplied build standard**, so every spec line is marked 🔧 MECHANICS (rely on it) or 📐 CONVENTION (an AI invention standing in for the firm — **never quote it as what their firm does**). **8 planted traps T1–T8, do-not-fix before they fire** — ⚠️ **but the plan has MOVED as of Aug 20: T5 and T8 have FIRED (J-P9/J-P10), T4 CANNOT fire because the vendor now requires the token, and T1 is deferred to Part 4/5. Read the Part 3 entry before assuming any trap is still pending**; T7 (a pinned, keystore-held key that still grants a full shell) fires *after* the deploy looks hardened. **A1/A2 settled: bookmark `http://192.168.1.185:8080/`, no DNS, no TLS.** No TLS is ledger row **J1** (accepted knowingly — a CI session cookie is a credential to everything Jenkins can deploy). **No DNS is cosmetic *because the address is static*** — the cost only appears if `.185` moves, when four things need hand-editing (Jenkins URL, webhook, agent, bookmark — the OAuth redirect is gone with A8); in a chapter that is a Lab-vs-PROD **table row, not a callout**, per CONVENTIONS' "wrong, not merely smaller" threshold. ⚠️ **T8 is NOT caused by skipping DNS** (the plan misattributed it once): GitLab blocks webhooks by **resolved address**, so a hostname on `192.168.1.185` is blocked identically. Two hard rules you cannot infer from the code: **Jenkins deploys the stack `capricorn-jenkins`, NEVER `capricorn`** (the Phase 16 GitLab pipeline stays alive as the comparison, and must be neutered rather than deleted if it is ever silenced), and 🚨 **B10 — Jenkins pushes images ONLY to the `lab` group, NEVER `production/*`, because QA `.180` and PROD `.184` pull `production/capricorn/<svc>:latest` and an overwrite there SHIPS TO PRODUCTION.** ⚠️ **Parts 4 and 5 were swapped Aug 20** — deploy precedes build so both CI systems ship identical artifacts; restoring the old order destroys the comparison |
...
- **🎉 ANDREW GOT THE JOB (confirmed Aug 12, 2026).** The interviews happened **Aug 6 and Aug 7** and
  ...
  🚨 **CROSS-PHASE PRINCIPLE (Sep 16, 2026) — CONFIDENT FORMATTING READS AS VERIFIED, SO EMPHASIS MUST
  NOT OUTRANK PROVENANCE.** A false sequencing rule survived a full plan review in Phase 18 because it
  was written as `🚨 Hard sequencing rule, and it is NOT optional` — the emphasis did the work that
  evidence should have done, and nobody re-derived it. ⭐ **The defence is the one this project already
  uses for chapter rows: mark the claim's ORIGIN (verified here / upstream docs / recited), and mark it
...
- **🔵 ACTIVE — Phase 18: Kubernetes HA + CKA prep** (`phases/phase18_k8s_cka_build.md`, opened Sep 16, 2026).
  📋 **PLAN WRITTEN, NOTHING BUILT, NOT YET APPROVED** — Andrew has not read it end to end, and the mandatory
  phase process requires that before any build work.
  **Two goals with separate finish lines, and they must not be re-merged:** (1) learn to deploy/install/
  configure Kubernetes **for work**, written up as a repeatable procedure; (2) pass the **CKA**. ⭐ Blending
  them in the first draft produced a strong HA build that covered barely half the exam.
  **Four stages, Andrew's numbering:** **0** prepare five VMs · **1** install and configure Kubernetes step
  by step · **2** sign off + snapshot the baseline · **3** CKA drilling (~10 exercises).
  **Spec:** VMIDs **201–205 → `.201–.205`**, kube-vip VIP **`.206` (no VM behind it)**, 2 vCPU / 4 GB / 40 GB
  each on `vm-ephemeral`, **`kubeadm` v1.35 → v1.36**, **Calico**, 3 control planes w/ stacked etcd + 2 workers.
  ⛔ **No planted traps** (deliberate `METHOD.md` deviation) and ⛔ **no leaked/recalled exam content** — the
  latter breaches the Linux Foundation confidentiality agreement and can invalidate a certification; exercises
  come from the published CNCF curriculum, ⭐ which is what exam questions are generated from anyway.
  📖 Research + the education track share one folder: **`education/k8s-cka-prep/`**. **Everything hazardous is
  in the PHASE INDEX row above — read it before touching this.**

- **⏸️ ON HOLD — Phase 17: Jenkins** (`phases/phase17_jenkins.md`, started Aug 19, 2026).
  🙋 **Andrew paused it on Sep 16, 2026 to start a new project.** ⛔ **Paused, NOT closed and NOT
  superseded** — every hard rule below still binds (above all **B10**), the traps stay unfixed, and
  the resume point is **Part 4 (deploy to the Swarm)**. Do not "tidy up" or finish anything here on
  your own initiative while it is on hold; the half-built state IS the recorded state.
...
⚠️ **State: Kubernetes PACKAGES installed and INERT** (v1.35.8, apt-held; containerd 2.3.5 with CRI
enabled; `crictl` 1.35.0 + `/etc/crictl.yaml`; `containerd.io` and `cri-tools` also HELD) — **but NO
CLUSTER.** 🔻 **CORRECTED Sep 17, 2026: the kubelet is NOT `inactive` — it is `activating
(auto-restart)`, restarting every ten seconds on a missing `/var/lib/kubelet/config.yaml`** until
`kubeadm init`/`join` writes it. **The earlier `inactive` was an artefact of a node that had not
rebooted since install.** ✅ **Snapshot `c01-nodes-ready` on all five, taken Sep 17 after a
deliberate reboot onto kernel 6.8.0-139** — the Sep 16 record claimed this snapshot and it had never
been taken.
📖 **Built by the three scripts in `education/k8s-cka-prep/scripts/`** — `1-provision-vms.sh` (Proxmox,
only its contract is portable), `2-personalize.sh` (environment; portable), `3-k8s-base.sh` (kubeadm-ready,
assigns NO role; portable). All three take `--check` and are idempotent — a second run must report **0
changes**. See that folder's `README.md`. ⚠️ **`k8s-setup.sh` was deleted** — its content lives in 2 and 3.
...
mistake.** ⭐ **So the real recurring risk here was never DHCP; it was a human typing a static address into
the range the lab uses.** That is exactly why the Phase 18 reservation is written into this file: the
protection against it is a *record*, not a router setting.
⚠️ **Still true and worth keeping: three household devices sit at `.215`, `.217` and `.220` — BELOW the
pool.** They are holding leases issued *before* the pool was narrowed and are keeping them, the same
mechanism that kept the Denon on `.202` through three reboots. ⛔ **Do not allocate lab addresses in
`.207–.220`** without sweeping first. **`.201–.206` are clear and nothing new will be offered there**, because
the DHCP server only hands out from its pool.
...
| `phases/phase18_k8s_cka_build.md` | 🔵 **CURRENT PHASE** — the 5-node kubeadm/CKA build. Plan only, nothing built, not yet approved |
| `education/k8s-cka-prep/` | CKA research: exam environment, curriculum + weights, logistics, lab-parity decisions, sources with contradictions marked |
| `phases/phase17_jenkins.md` | ⏸️ **ON HOLD since Sep 16, 2026** (paused, not closed). Hard rules B10/B11 still bind, traps T1–T8 still unfixed. Resume at Part 4 |
