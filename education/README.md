...
| [**jenkins**](jenkins/README.md) | Jenkins: a controller built from nothing, wired to the same GitLab and deploying the same application to the same Swarm — then made to fail on purpose | 3 written | **Yes** — a real controller and agent, aimed at the registry and cluster track 2 already uses | ⏸️ **ON HOLD since Sep 16, 2026** (paused, not closed) — Ch1–3 written, rest planned in [`phase17`](../phases/phase17_jenkins.md) |
| [**k8s-cka-prep**](k8s-cka-prep/README.md) | Kubernetes done properly: a five-node `kubeadm` cluster with an HA control plane behind kube-vip — built as a procedure to repeat at work — then used to drill the **CKA** curriculum one administrative task type per chapter | 1 written | **Yes** — five real kubeadm-ready nodes, built and verified twice (rebuilt baseline Sep 17) | 🔵 **ACTIVE** — Ch1 written and revised, Stage 1 (kube-vip + `kubeadm init`) next; planned in [`phase18`](../phases/phase18_k8s_cka_build.md). ⚠️ Holds **exam research as well as chapters**, deliberately |
