...
| [**k8s-cka-prep**](k8s-cka-prep/README.md) | Kubernetes done properly: a five-node `kubeadm` cluster with an HA control plane behind kube-vip — built as a procedure to repeat at work — then used to drill the **CKA** curriculum one administrative task type per chapter | 0 written (research only) | **Planned** — five VMs, not yet built | 📋 Planned in [`phase18`](../phases/phase18_k8s_cka_build.md). ⚠️ Holds **exam research as well as chapters**, deliberately |
