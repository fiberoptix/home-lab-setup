...
| 06 | [False greens](chapter06_false_greens.md) | ⭐ The unplanned capstone: nine ways this cluster reported success for a question nobody asked, why every one of those signals was *honest*, the ladder of questions, and the smoke gate we built — including the failure it missed and how the gap was closed. 🚨 The ninth runs the pattern **backwards**: `docker service ps`'s `Running 8 hours ago` is the manager's last status *stamp*, not the task's age, and reading it as an age **invented a data-loss incident that never happened** (§5, 🤖 AI-measured Aug 19) | ✅ Written |
...
tooling, and in six of our own experiments, which made it a subject rather than a footnote.

🤖 **Two pieces of this track were executed by the AI rather than by Andrew, and both say so where they
appear.** Chapter 7's commands were run by the AI at his written instruction, and so was the sixth case in
chapter 6 §5 — the one that closed Phase 16's last open measurement on Aug 19. Every output in both is
real, but the operator judgement behind them was not earned by hand, so they are deliberately marked as
weaker rather than quietly shelved alongside the rest. Chapters 1–6 are otherwise his at the keyboard.
