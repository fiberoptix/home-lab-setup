...
# ------------------------------------------------------------------ footer
#
# A bare page number, centred in the bottom margin, so a printed chapter can be
# reassembled after it is dropped. Added Aug 13, 2026 at Andrew's request -- he
# had been inserting them by hand in Word before every print.
#
# Just the digit: no "Page", no "of N", no running title. The chapter's H1
# already names the track (see CONVENTIONS.md), and anything more in the
# footer is furniture on a page that is mostly technical prose.
#
...
def field(instr, placeholder):
    ...
    which is why this build has no table of contents -- PAGE is evaluated
    during layout, so it renders on open and on print."""
...
def add_footer(work):
    (work / "word" / FOOTER_PART).write_text(footer_xml(), encoding="utf-8")
...
def build_reference(dest):
    with tempfile.TemporaryDirectory() as td:
        ...
        dp.write_text(patch_sectpr(dp.read_text(), footer=True), encoding="utf-8")
