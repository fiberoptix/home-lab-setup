...
**Status:** IN PROGRESS — Parts 1, 2, 3 and **4 COMPLETE** (Jul 25–27), plus a **topic-provisioning session (Aug 3)**. **Redpanda is live: 3 brokers, healthy 3/3, topics `orders` / `executions` / `market-ticks` (6 partitions, RF 3), quorum and failure drills run and documented.** Education series: Chapters 1 (k3s, 846 lines), 2 (object model / rollouts / probes, 631 lines), 3 (Redpanda, 1023 lines) and 4 (provisioning application state, 823 lines) written; Chapters 2–4 double as replayable runbooks, with tested artefacts committed at `education/manifests/` (`redpanda-values.yaml`, `web-deployment.yaml`, `seed-topics.sh`, `seed-topics-job.yaml`). **Headline finding from Aug 3: pod-Ready is not cluster-ready — a measured 9-second window where all 3 brokers were `2/2 Ready` and 11 of 18 partitions were leaderless.** **Restore point: `qm rollback 186 s03-redpanda-up`** (Jul 27 14:58 — `s02-k3s-up` predates Redpanda and rolling back that far wipes it; `s03` predates the `orders`/`executions` topics). **Still outstanding in Part 4: Redpanda Console (ClusterIP-only, needs a port-forward) and Schema Registry.** **Next: consumer groups + rebalancing, then Part 6 (the Python app). Interview ~Aug 1.**
