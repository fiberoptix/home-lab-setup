...
- 🆕 **Ledger rows J7 and J8**, for compromises that had been taken but never written down —
  **J7** host keys pinned by hand, **J8** 🚨 **the cloned repo contains plaintext credentials.**
  J8 is `✅ verified`; it is T5 promoted from a trap to a standing compromise.
- 📌 **The live-mirror measurement.** `PASSWORDS.md` in the agent workspace was **14,854 B** in J-P9
  and is **16,781 B** now, because the doc push re-checked it out. ⭐ **So the notifyCommit token
  created in J-P10 was already in both copies within the hour** — T5 is not a stale artefact of one
  bad checkout, it is a mirror that tracks the secrets file, and **every future credential lands
  there automatically.**
...
|---|---|---|---|---|---|
| **J8** | 🚨 **The repository Jenkins clones contains plaintext credentials.** `PASSWORDS.md`, `github_credentials.md` and the live Swarm private key `working/phase16/swarm_deploy_ed25519` are committed to the private GitLab mirror on purpose. Inherited from Phase 16; **first became a CI problem on Aug 20, 2026** when T5 fired (J-P9). | The mirror is PRIVATE, `push_github.sh` keeps all of it off the public remote, and **one clone reproduces the whole lab** — which is the point of the repo. The credentials are lab-only by rule. | Secrets live in a secret manager; the repo holds **references**, and CI fetches them at run time scoped to the job, so a clone is worth nothing. | 🚨 **Every system that clones the repo becomes a copy of the vault** — build agents, laptops, backups, and the controller SCM cache nobody knew existed. ⭐ **Access control on the pipeline is irrelevant when the artefact it clones is the vault.** ⚠️ And it is *live*: `PASSWORDS.md` grew 14,854 → 16,781 B between two builds, so **each new credential arrives automatically.** | ✅ **verified** — files, sizes and modes measured on both machines (J-P9, Ch3 §6) |
| **J7** | **Git host keys are pinned by hand**, pasted as two literal lines into *Git Host Key Verification Configuration*. Aug 20, 2026. | One git server that will not be rebuilt, and ⭐ **the keys were read from `.181`'s own `/etc/ssh/` over an already-trusted session** — a real pin, not `ssh-keyscan` asking the unidentified host to vouch for itself. | Known-hosts material is distributed by configuration management, or host keys are signed by an internal CA, so **a rebuilt server is trusted automatically and a substituted one still is not**. | ⚠️ **Manually-pinned keys rot silently.** When the git server is rebuilt, every Jenkins fails closed at once — **correct behaviour that is indistinguishable from an outage** — and the pressure in that moment is to switch verification off "temporarily". 🚨 That is how a pin becomes permanently absent. | ✅ **verified** — strategy is `ManuallyProvidedKeyVerificationStrategy`, both ed25519 and RSA present with the IP alias |
