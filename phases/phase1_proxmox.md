...
⚠️ **Its pool numbers are a Jan 14, 2026 reading and have moved a long way since** — `vm-critical`
was later measured to be dominated by `refreservation` rather than data, `vm-ephemeral` was rebuilt
with `ashift=12` on Jul 9, 2026, and the host went to 192 GB RAM on Aug 26, 2026. Read them as a
dated measurement, not as current state; `MEMORY.md` → STORAGE is current.
