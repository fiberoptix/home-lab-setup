...
# Whether the server is even consulted is decided further down by the offline
# check, so do not claim it here -- printing a server that is never contacted is
# how you spend an hour debugging a network path the run never used.
echo "Script Server: $SCRIPT_SERVER (only if sub-scripts are not already local)"
echo ""
...
# ============================================
# ---------------------------------------------------------------------------
# OFFLINE (self-contained) MODE -- added Aug 26, 2026.
#
# WHY THIS EXISTS. The script server lives at 192.168.1.195, which is a VMware
# guest on the Z8 workstation. Dual-booting that same Z8 into Fedora means
# Windows is not running, so VMware is not running, so the script server is
# DOWN -- at precisely the moment a fresh Fedora install wants to fetch these
# scripts from it. The server and the client are the same physical machine.
# That is not a fixable ordering problem; the build simply has to work with no
# lab network at all.
#
# So: if all the sub-scripts are ALREADY sitting next to this file, use them and
# skip the network entirely. That makes a USB stick (or any copied folder) a
# complete, self-contained build kit.
#
# The check is deliberately ALL-OR-NOTHING. A partial set falls back to
# downloading, because a half-populated directory is far more likely to be a
# broken copy than an intended offline kit -- and silently running 4 real
# scripts plus 3 stale ones is the kind of failure that looks like it worked.
# ---------------------------------------------------------------------------
OFFLINE=0
MISSING=""
...
if [ "$OFFLINE" -eq 1 ]; then
    echo "-- OFFLINE MODE: all $(echo $SCRIPTS | wc -w) sub-scripts found in $SCRIPT_DIR"
    ...
    echo ""
    # NOTE: the script-library guard below is deliberately NOT applied here.
    # It exists because DOWNLOADING into the library destroys it. Offline mode
    # writes nothing at all, so there is no hazard to guard against.
else
    ...
    # the Ubuntu orchestrator did exactly this and zeroed all eight of its own
    # files.
    #
    # `curl -o <file>` truncates the destination as it opens it, and nginx
    # serves www/fedora/ straight off disk, so running here would empty each
    # file and then read back the zero bytes just written. This tree escaped
    # the actual incident only by luck -- the distro guard above happens to
    # exit first on a Debian host, which is where the testing was being done.
    ...
    for script in $SCRIPTS; do
        ...
        # Temp file, then move on success. `curl -o dest` truncates dest up front,
        # so a failed download REPLACES A WORKING SCRIPT WITH AN EMPTY FILE --
        # worst in the re-run-to-repair case this script exists for.
    ...
    if [ $DOWNLOAD_FAILED -eq 1 ]; then
        ...
        echo "copy www/fedora_local/ to this machine and run host_setup.sh from"
        echo "inside it. It carries all the sub-scripts and needs no script server."
        exit 1
    fi
