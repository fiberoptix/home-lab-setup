...
# ============================================
# OFFLINE MODE vs DOWNLOAD MODE
# ============================================
# ---------------------------------------------------------------------------
# OFFLINE (self-contained) MODE -- added Aug 26, 2026, mirroring the Fedora tree.
#
# WHY. The script server at 192.168.1.195 is a VMware guest on the Z8
# workstation. Any build that requires that same physical box to be running
# something else -- dual-booting it, or reinstalling it -- cannot fetch from it.
# The server and the client are one machine. Carrying the scripts in is the only
# answer, so a copied folder must be a complete build kit.
#
# ⚠️ UBUNTU CARRIES ONE MORE FILE THAN FEDORA. `anysphere.gpg` is in $SCRIPTS
# because Cursor's official apt key URL returns 403, so this tree mirrors the key
# locally. An Ubuntu offline kit is therefore NOT just the .sh files -- omit the
# key and Cursor's repo fails to verify. The all-or-nothing check below covers it
# automatically, because the key is a member of $SCRIPTS like everything else.
#
# The check is ALL-OR-NOTHING on purpose. A partial set falls back to
# downloading, because a half-populated directory is far more likely to be a
# broken copy than an intended kit -- and silently running some real scripts
# alongside some stale ones is a failure that looks like success.
# ---------------------------------------------------------------------------
OFFLINE=0
MISSING=""
for script in $SCRIPTS; do
    [ -s "${SCRIPT_DIR}/${script}" ] || MISSING="$MISSING $script"
done
if [ -z "$MISSING" ]; then
    OFFLINE=1
fi
...
if [ "$OFFLINE" -eq 1 ]; then
    echo "╔════════════════════════════════════════╗"
    echo "║  OFFLINE MODE - using local files      ║"
    echo "╚════════════════════════════════════════╝"
    echo ""
    echo "  All $(echo $SCRIPTS | wc -w) files found in $SCRIPT_DIR."
    echo "  Nothing will be downloaded; the script server is not needed."
    echo ""
    for script in $SCRIPTS; do
        printf '  using  %-22s (%s bytes)\n' "$script" "$(wc -c < "${SCRIPT_DIR}/${script}")"
    done
    echo ""
    # The script-library guard below is deliberately NOT applied in this branch.
    # It exists because DOWNLOADING into the library destroys it; offline mode
    # writes nothing, so there is nothing to protect against.
else
    echo "╔════════════════════════════════════════╗"
    ...
    echo ""
    if [ -n "$MISSING" ]; then
        echo "  (not offline: missing$MISSING)"
        echo ""
    fi
    ...
    #
    # `wget -O <file>` TRUNCATES the destination the moment it opens it, before
    # the transfer. nginx serves www/ubuntu/ straight off disk via a bind mount,
    # so running this script with SCRIPT_DIR == www/ubuntu means wget empties the
    # file and *then* asks nginx for it -- and nginx reads back the zero bytes
    # wget just wrote. All seven sub-scripts plus anysphere.gpg went to 0 bytes
    # in one run. host_setup.sh survived only because it is not in its own list.
    #
    # ⭐ The general shape: a program that fetches its inputs into its own source
    # directory, from a server backed by that same directory, will eat itself.
    # Detect it rather than document it -- the source tree is identifiable by the
    # two files that sit beside it.
    # -----------------------------------------------------------------------
    ...
    for script in $SCRIPTS; do
        ...
        #
        # `wget -O dest` truncates dest before the transfer, so a failed or
        # partial download REPLACES A WORKING SCRIPT WITH AN EMPTY FILE. That
        # matters most in the case this script is designed for: re-running on a
        # host to repair it. A momentary network blip would have left the box
        # with eight empty scripts and no way to tell that was what happened.
    ...
    if [ $DOWNLOAD_FAILED -eq 1 ]; then
        ...
        echo ""
        echo "NO NETWORK TO THE LAB? Use the self-contained USB kit instead --"
        echo "copy www/ubuntu_local/ to this machine and run host_setup.sh from"
        echo "inside it. It carries every sub-script AND anysphere.gpg."
        exit 1
    fi
